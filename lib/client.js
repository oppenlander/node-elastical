...
Client.prototype = {
    ...
    **/
    _SEARCH_PARAMS: [
        // Removed 'scroll_id' due to hitting 4096 URL char limit
        'preference', 'routing', 'scroll', 'search_type', 'timeout', 'ignore_indices'
    ],
    ...
    **/
    ...
    /**
    Deletes a set of documents based on the query

    [ElasticSearch docs](http://www.elasticsearch.org/guide/en/elasticsearch/reference/current/docs-delete-by-query.html)

    @example

        var client = new require('elastical').Client();

        client.deleteBy({query: 'pie'}, function(err, results) {
            if (err) { throw err; }
            console.log(results);
       });

    @method deleteBy
    @param {Object} options Delete By options.
      @param {String} options.query Query for the subset of ducments to delete.
          This can be either a full query object, or a query string.
      ...
      @param {String|String[]} [options.type] Type name or array of type names
		  ...
		  @param {String} [options.df] Extra Query String paramerter. The default
		      field used when no field prefix is defined within the query. 
		  @param {String} [options.analyzer] Extra Query String paramerter. The
		      analyzer name to be used when analyzing the query string.
		  @param {String} [options.default_operator='OR'] Extra Query String paramerter.
		      The default operator to be used, can be `AND` or `OR`.
    @param {Function} callback Callback function.
      @param {Error|null} callback.err Error, or `null` on success.
      @param {Object} callback.res Full ElasticSearch response data.
    **/
    deleteBy: function (options, callback) {
      var url   = '',
          body  = {};
      ...
      if (typeof options.query === 'string') {
        url += '?q=' + options.query;
				...
				// Additional query parameters with query string
				if(options.df) {
					url += '&df=' + options.df;
				}
				if(options.analyzer) {
					url += '&analyzer=' + options.analyzer;
				}
				if(options.default_operator) {
					url += '&default_operator=' + options.default_operator;
				}
      } else if (typeof options.query === 'object') {
        body.query = options.query;
      } else {
        return callback(new Error('options.query is required for Delete By')), undefined;
      }
      ...
      this._request(url, {
        ...
        if(err) { return callback(err, res), undefined; }
        callback(null, res);
      });
    },
...
/**
    ...
    **/
    search: function (options, callback) {
        ...
        // Fix 4096 URL char limit error on scroll
        // We can only pass scroll_id in the body as a plain text string
        if (options.scroll_id != null) {
          this._request(url, {
              method: 'POST',
              body: options.scroll_id
          }, function (err, res) {
    ...
    _request: function (path, options, callback) {
        ...
        request(url, options, function (err, res, body) {
            if(err && err.code == 'EADDRNOTAVAIL') {
                console.log('Got EADDRNOTAVAIL, trying again in 10 seconds');
                setTimeout(function() {
                    self._request(path, options, callback);
                }, 10000);
                return;
            }
